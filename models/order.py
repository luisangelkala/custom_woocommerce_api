class SaleOrderLine(models.Model):
    ...
    # ---------- ONCHANGE NUEVO (Paso 3) ----------
    @api.onchange('product_id')
    def _onchange_product_id_apply_brand_discount(self):
        """
        Al seleccionar un producto, copia automáticamente el descuento de marca
        sincronizado desde WordPress (x_brand_discount) al campo de la línea.
        """
        for line in self:
            if line.product_id:
                # Copiar el valor del producto al pedido
    ...
    @api.depends(
        ...
        'discount_price'  # <--- IMPORTANTE: Agregado para que recalcule si cambia el descuento
    )
    def _compute_price_quote(self):
        for line in self:
            ...
            # print(f"Computing price_quote for line {line.id}: ...") 
            ...
            try:
                ...
                # Tu lógica de descuento original
